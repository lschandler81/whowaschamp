...
export default async function BoxingThisWeekPage({ searchParams }: { searchParams?: Record<string, string> }) {
  ...
  const events = await getEventsForIsoWeek('boxing', isoYear, isoWeek);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white py-12 md:py-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Boxing — This Week’s Events</h1>
        </div>
        <p className="text-sm text-gray-600 mb-8">{formatRangeGB(start.toISOString().slice(0,10), end.toISOString().slice(0,10))}</p>

        <div className="mb-6 flex items-center gap-4 text-sm">
          <Link href={`/boxing/this-week?delta=${delta - 1}`} className="text-gray-700 hover:text-red-600">Last week</Link>
          <Link href={`/boxing/this-week?delta=0`} className="text-gray-700 hover:text-red-600">This week</Link>
          <Link href={`/boxing/this-week?delta=${delta + 1}`} className="text-gray-700 hover:text-red-600">Next week</Link>
        </div>
        ...
        <Breadcrumbs
          ...
          items={[
            ...
            { label: 'Boxing — This Week’s Events' },
          ]}
        />
      </div>
    </div>
